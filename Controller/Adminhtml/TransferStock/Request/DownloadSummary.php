...
class DownloadSummary extends \Magestore\InventorySuccess\Controller\Adminhtml\TransferStock\AbstractRequest
{
    ...
    public function execute()
    {
        ...
        $data = array(
            array('Name','SKU','REQUESTED QTY', 'QTY DELIVERED', 'QTY RECEIVED','QTY RETUNED')
        );
        $data = array_merge($data, $this->generateDownloadData());
    ...
    public function generateDownloadData() {
        ...
        foreach ($productCollection as $product) {
            $data[]= array($product->getData('product_name'), $product->getData('product_sku'), $product->getData('qty'), $product->getData('qty_delivered'), $product->getData('qty_received'),$product->getData('qty_returned'));
        }
        return $data;
    }
